namespace GrpcPokemon
{
    ...
    public static class ConsoleDisplay
    {
        ...
        public static void WriteTextInCentre(string textToWrite) => Console.Write(string.Format("{0," + ((Console.WindowWidth / 2) + (textToWrite.Length / 2)) + "}", textToWrite));
        ...
        public static void WriteGenericMenu(List<MenuItems> options, MenuItems selectedOption, OptionalText optionalText)
        {
            ...
            List<MenuItems> OMax = options.OrderBy(o => o.Name.Length).Reverse().ToList();
            ...
            foreach (MenuItems option in options)
            {
                ...
                WriteTextInCentre($"        ╭──{new String('─', OMax[0].Name.Length)}──╮        {Environment.NewLine}");
                WriteTextInCentre($"        │  {option.Name.PadRight(OMax[0].Name.Length)}  │ {(option == selectedOption ? "(Enter)" : "       ")}{Environment.NewLine}");
                WriteTextInCentre($"        ╰──{new String('─', OMax[0].Name.Length)}──╯        {Environment.NewLine}");
                Console.ResetColor();
            }
        }
        ...
        private static void WriteOptionalText(OptionalText optionalText)
        {
            if (optionalText is not null && optionalText.PrimiaryText is not "")
            {
                ...
                string[] lines = optionalText.PrimiaryText.Split('\n');
                for (int i = 0; i < lines.Length; i++)
                {
                    Console.WriteLine(PadBoth(lines[i], Console.BufferWidth));
                }
